...
The `/statusz` endpoint reports operational JSON, including:

* gateway version and uptime
* request/cache/proxy counters
* active background mirror jobs
* recent completed mirror jobs
* recent failed mirror jobs with errors
* submitted/completed/failed/rejected/deduplicated job counts
* queue capacity information
* GitLab base URL and mirror root group
...
The `/metrics` endpoint is exposed with the official Python `prometheus-client` library and includes request counters, request-duration histograms, cache hit/miss counters, proxy byte counters, mirror-job counters, mirror-job duration histograms, queue gauges, and active-job gauges.

---

## Observability

Git Cache Gateway exposes two built-in observability endpoints.

### `/statusz`

`/statusz` is intended for humans and lightweight automation. It returns JSON with runtime information such as gateway version, uptime, cache hit/miss counters, proxy counters, active mirror jobs, and recent mirror-job success/failure history.
...
`/metrics` is intended for Prometheus scraping. It is generated by `prometheus-client`, so the output follows the normal Prometheus exposition format.

Example Prometheus scrape config:

```yaml
scrape_configs:
  - job_name: git-cache-gateway
    static_configs:
      - targets:
          - git-cache.example.local:8080
```

Important metrics include:

```text
git_cache_gateway_build_info
...
- Added official `prometheus-client` dependency.
- Added `/metrics` Prometheus endpoint.
- Added request, cache, proxy byte, and mirror-job metrics.
- Added mirror-job queue and active-job gauges.
- Expanded `/statusz` with version, uptime, active jobs, recent completed jobs, and recent failures.
