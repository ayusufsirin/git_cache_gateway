## v0.2.4 build-time CA support for pip

Company TLS interception can break `pip install` during `docker compose build`, because the runtime entrypoint has not run yet. v0.2.4 installs extra CA certificates during image build as well.

Put your company/internal CA certificate in the project-root `ca/` directory before building:
...
The Dockerfile installs any `ca/*.crt` or `ca/*.pem` into the OS trust store before running:
...
The same `ca/` directory is also mounted at runtime by the example Compose file, so Python HTTPS calls and Git mirror operations use the same trusted CA.

If you use the example from `examples/docker-compose.yml`, put the cert in `git-cache-gateway/ca/`, not `git-cache-gateway/examples/ca/`, because the Docker build context is the project root.

# git-cache-gateway

## v0.2.3 CA certificate support

The gateway can now use a company/internal CA certificate without disabling TLS verification. The Docker entrypoint automatically installs any `*.crt` or `*.pem` files mounted under `/etc/git-cache-gateway/ca` into the container trust store before starting the server. This is the recommended path because Python, Git, and curl-style HTTPS calls all keep the normal public CA store plus your company CA.
...
[tls]
# Usually empty when using the Docker auto-install CA directory.
ca_file = ""
ca_path = ""
```
...
Put your PEM/CRT file here:

```text
./ca/company-ca.crt
```

Then rebuild/restart and check:

```bash
docker compose up -d --build
docker logs git-cache-gateway
docker exec -it git-cache-gateway git-cache-gateway doctor
```

Advanced alternative: if you do not want to install the CA into the OS store, set `[tls].ca_file = "/etc/git-cache-gateway/ca/company-ca.crt"`. The gateway will pass that CA explicitly to Python HTTPS calls and Git mirror commands. Use `verify_tls = false` only as a temporary diagnostic fallback.
...
event=cache_miss client=example.local id=12 remote=https://github.com/octocat/Hello-World.git mirror=https://gitlab.example.local/mirror/github.com/octocat/Hello-World.git
