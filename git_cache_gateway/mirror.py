...
from .util import inject_basic_auth, run
...
class MirrorManager:
    ...
    def __init__(self, cfg: Config):
        self.cfg = cfg
        self.gitlab = GitLabAPI(cfg.gitlab, cfg.token)
    ...
    def _upstream_git_env(self) -> dict[str, str] | None:
        return {"GIT_SSL_NO_VERIFY": "true"} if not self.cfg.upstream.verify_tls else None

    def _gitlab_git_env(self) -> dict[str, str] | None:
        return {"GIT_SSL_NO_VERIFY": "true"} if not self.cfg.gitlab.verify_tls else None
